...
clearInterval(opt.aktKenInterval);clearInterval(opt.timerinterval);clearInterval(opt.waitForWF);clearTimeout(opt.resetAll);opt.width=top_container.width()-opt.hCorrection;if(!opt.fixHeight||!opt.fixHeight=="true"){opt.height=opt.width*opt.whproportion;opt._propw=opt.width/opt.originalwidth;opt._proph=opt.height/opt.originalheight;}
if(opt.width<500&&responsive&&!opt.fixHeight){opt._propw=opt._propw*1.7;opt._proph=opt._proph*1.7;}
opt.preloadedSlides=200;top_container.empty();top_container.remove();$('body').find('#'+opt.oldid).append(opt.fullHTML);top_container=$('body').find('#'+opt.oldid).find('div:first');top_container.height(opt.height+opt.vCorrection);opt.resetAll=setTimeout(function(){startToPrepare(top_container,opt);},200);});})}})
...
function prepareSlidesContainer(top,opt){top.append('<div class="kenburn-bg" style="z-index:7;position:absolute;top:0px;left:0px;width:'+opt.width+'px;height:'+opt.height+'px;overflow:hidden"></div>');var bg=top.find('.kenburn-bg');opt.padtop=0;opt.padleft=0;opt.padright=0;opt.padbottom=0;opt.botop=0;opt.boleft=0;opt.boright=0;opt.bobottom=0;if($.browser.msie){if(bg.css('paddingTop')!=undefined&&bg.css('paddingTop')!=null)
if(bg.css('paddingTop').length>0)
opt.padtop=parseInt(bg.css('paddingTop'),0);if(bg.css('paddingLeft')!=undefined&&bg.css('paddingLeft')!=null)
if(bg.css('paddingLeft').length>0)
opt.padleft=parseInt(bg.css('paddingLeft'),0);if(bg.css('paddingRight')!=undefined&&bg.css('paddingRight')!=null)
if(bg.css('paddingRight').length>0)
opt.padright=parseInt(bg.css('paddingRight'),0);if(bg.css('paddingBottom')!=undefined&&bg.css('paddingBottom')!=null)
if(bg.css('paddingBottom').length>0)
opt.padbottom=parseInt(bg.css('paddingBottom'),0);if(bg.css('borderTopWidth')!=undefined&&bg.css('borderTopWidth')!=null)
if(bg.css('borderTopWidth').length>0)
opt.botop=parseInt(bg.css('borderTopWidth'),0);if(bg.css('borderLeftWidth')!=undefined&&bg.css('borderLeftWidth')!=null)
if(bg.css('borderLeftWidth').length>0)
opt.boleft=parseInt(bg.css('borderLeftWidth'),0);if(bg.css('borderRightWidth')!=undefined&&bg.css('borderRightWidth')!=null)
if(bg.css('borderRightWidth').length>0)
opt.boright=parseInt(bg.css('borderRightWidth'),0);if(bg.css('borderBottomWidth')!=undefined&&bg.css('borderBottomWidth')!=null)
if(bg.css('borderBottomWidth').length>0)
opt.bobottom=parseInt(bg.css('borderBottomWidth'),0);}else{if(bg.css('paddingTop').length>0)
...
opt.padbottom=parseInt(bg.css('paddingBottom'),0);if(bg.css('borderTopWidth').length>0)
opt.botop=parseInt(bg.css('borderTopWidth'),0);if(bg.css('borderLeftWidth').length>0)
opt.boleft=parseInt(bg.css('borderLeftWidth'),0);if(bg.css('borderRightWidth').length>0)
opt.boright=parseInt(bg.css('borderRightWidth'),0);if(bg.css('borderBottomWidth').length>0)
...
if(item.index()!=newitem.index()){setTimeout(function(){item.find('.canvas-now-bw').css({'visibility':'hidden'});},0);}
