...
/**
 * Base class that represents a row from the 'newstypes' table.
 *
 * 
 *
 ...
 */
abstract class BaseNewstypes extends BaseObject  implements Persistent {
	 ...
	 * @return     Newstypes The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = NewstypesPeer::NUM_COLUMNS - NewstypesPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Newstypes object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = NewstypesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseNewstypes:delete:pre') as $callable)
			{
			...
			if ($ret) {
				NewstypesPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseNewstypes:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseNewstypes:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseNewstypes:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				NewstypesPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($has_non_empty_fields || ($only_service_fields_modified && !$is_i18n)) {
						$pk = NewstypesPeer::doInsert($this, $con);
					...
					$affectedRows += NewstypesPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = NewstypesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = NewstypesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = NewstypesPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NewstypesPeer::ID)) $criteria->add(NewstypesPeer::ID, $this->id);
		if ($this->isColumnModified(NewstypesPeer::NAME)) $criteria->add(NewstypesPeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(NewstypesPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Newstypes (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Newstypes Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     NewstypesPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new NewstypesPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this Newstypes has previously been saved, it will retrieve
	 * related Newss from storage. If this Newstypes is new, it will return
		...
		if ($this->collNewss === null) {
			if ($this->isNew()) {
			   $this->collNewss = array();
				...
				$criteria->add(NewsPeer::TYPE, $this->id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseNewstypes:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseNewstypes::%s', $method));
	  }
