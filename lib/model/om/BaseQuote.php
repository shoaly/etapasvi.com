...
abstract class BaseQuote extends BaseObject  implements Persistent {
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collQuoteI18ns.
	 */
	private $lastQuoteI18nCriteria = null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseQuote:delete:pre') as $callable)
			{
			...
			if ($ret) {
				QuotePeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseQuote:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseQuote:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseQuote:save:post') as $callable)
				{
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($has_non_empty_fields || ($only_service_fields_modified && !$is_i18n)) {
						$pk = QuotePeer::doInsert($this, $con);
					...
					$affectedRows += QuotePeer::doUpdate($this, $con);
				}

				$this->resetModified(); // [HL] After being saved an object is no longer 'modified'
			}

			if ($this->collQuoteI18ns !== null) {
				foreach ($this->collQuoteI18ns as $referrerFK) {
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(QuotePeer::ID, $this->id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getQuoteI18ns() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addQuoteI18n($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collQuoteI18ns collection (array).
	 *
	 ...
	 * @see        addQuoteI18ns()
	 */
	public function clearQuoteI18ns()
	{
		$this->collQuoteI18ns = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collQuoteI18ns collection (array).
	 *
	 * By default this just sets the collQuoteI18ns collection to an empty array (like clearcollQuoteI18ns());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initQuoteI18ns()
	{
		$this->collQuoteI18ns = array();
	}

	/**
	 * Gets an array of QuoteI18n objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Quote has previously been saved, it will retrieve
	 * related QuoteI18ns from storage. If this Quote is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array QuoteI18n[]
	 * @throws     PropelException
	 */
	public function getQuoteI18ns($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(QuotePeer::DATABASE_NAME);
		}
		...
		if ($this->collQuoteI18ns === null) {
			if ($this->isNew()) {
			   $this->collQuoteI18ns = array();
				...
				$criteria->add(QuoteI18nPeer::ID, $this->id);

				QuoteI18nPeer::addSelectColumns($criteria);
				$this->collQuoteI18ns = QuoteI18nPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				QuoteI18nPeer::addSelectColumns($criteria);
				if (!isset($this->lastQuoteI18nCriteria) || !$this->lastQuoteI18nCriteria->equals($criteria)) {
					$this->collQuoteI18ns = QuoteI18nPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastQuoteI18nCriteria = $criteria;
		return $this->collQuoteI18ns;
	}

	/**
	 * Returns the number of related QuoteI18n objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related QuoteI18n objects.
	 * @throws     PropelException
	 */
	public function countQuoteI18ns(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(QuotePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collQuoteI18ns === null) {
			if ($this->isNew()) {
				...
				$count = QuoteI18nPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastQuoteI18nCriteria) || !$this->lastQuoteI18nCriteria->equals($criteria)) {
					...
					$count = count($this->collQuoteI18ns);
				}
			} else {
				$count = count($this->collQuoteI18ns);
			}
	...
	/**
	 * Method called to associate a QuoteI18n object to this object
	 * through the QuoteI18n foreign key attribute.
	 *
	 * @param      QuoteI18n $l QuoteI18n
	 * @return     void
	 * @throws     PropelException
	 */
	public function addQuoteI18n(QuoteI18n $l)
	{
		if ($this->collQuoteI18ns === null) {
			$this->initQuoteI18ns();
		}
		if (!in_array($l, $this->collQuoteI18ns, true)) { // only add it if the **same** object is not already associated
			array_push($this->collQuoteI18ns, $l);
			$l->setQuote($this);
		}
	}
	...
	{
		if ($deep) {
			if ($this->collQuoteI18ns) {
				foreach ((array) $this->collQuoteI18ns as $o) {
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseQuote:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseQuote::%s', $method));
	  }
	...
	/**
	 * Returns the "title" value from the current {@link QuoteI18n}.
	 */
	public function getTitle($culture = null)
	{
	  return $this->getCurrentQuoteI18n($culture)->getTitle();
	}
	
	/**
	 * Sets the "title" value of the current {@link QuoteI18n}.
	 *
	 * @return Quote
	 */
	public function setTitle($value, $culture = null)
	{
	  $this->getCurrentQuoteI18n($culture)->setTitle($value);
	  return $this;
	}
	 ...
	 * @return QuoteI18n
	 */
	public function getCurrentQuoteI18n($culture = null)
	{
	  if (null === $culture)
	  {
	  ...
	  {
	    $object = $this->isNew() ? null : QuoteI18nPeer::retrieveByPK($this->getPrimaryKey(), $culture);
	    if ($object)
	    {
	      $this->setQuoteI18nForCulture($object, $culture);
	    }
	    else
	    {
	      $this->setQuoteI18nForCulture(new QuoteI18n(), $culture);
	 ...
	 */
	public function setQuoteI18nForCulture(QuoteI18n $object, $culture)
	{
	  $this->current_i18n[$culture] = $object;
	  $this->addQuoteI18n($object);
	}
