...
/**
 * Base class that represents a row from the 'photo_i18n' table.
 *
 * 
 *
 ...
 */
abstract class BasePhotoI18n extends BaseObject  implements Persistent {
	 ...
	 * @return     PhotoI18n The current object (for fluent API support)
	 */
	public function setUpdatedAtExtra($v)
	{
		...
		if ( $this->updated_at_extra !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = PhotoI18nPeer::UPDATED_AT_EXTRA;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 6; // 6 = PhotoI18nPeer::NUM_COLUMNS - PhotoI18nPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PhotoI18n object", $e);
		}
	}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aPhoto !== null && $this->id !== $this->aPhoto->getId()) {
			$this->aPhoto = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PhotoI18nPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BasePhotoI18n:delete:pre') as $callable)
			{
			...
			if ($ret) {
				PhotoI18nPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePhotoI18n:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePhotoI18n:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePhotoI18n:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				PhotoI18nPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aPhoto !== null) {
				if ($this->aPhoto->isModified() || ($this->aPhoto->getCulture() && $this->aPhoto->getCurrentPhotoI18n()->isModified()) || $this->aPhoto->isNew()) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($has_non_empty_fields || ($only_service_fields_modified && !$is_i18n)) {
						$pk = PhotoI18nPeer::doInsert($this, $con);
					...
					$affectedRows += PhotoI18nPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PhotoI18nPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PhotoI18nPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PhotoI18nPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PhotoI18nPeer::UPDATED_AT_EXTRA)) $criteria->add(PhotoI18nPeer::UPDATED_AT_EXTRA, $this->updated_at_extra);
		if ($this->isColumnModified(PhotoI18nPeer::TITLE)) $criteria->add(PhotoI18nPeer::TITLE, $this->title);
		if ($this->isColumnModified(PhotoI18nPeer::BODY)) $criteria->add(PhotoI18nPeer::BODY, $this->body);
		if ($this->isColumnModified(PhotoI18nPeer::AUTHOR)) $criteria->add(PhotoI18nPeer::AUTHOR, $this->author);
		if ($this->isColumnModified(PhotoI18nPeer::ID)) $criteria->add(PhotoI18nPeer::ID, $this->id);
		if ($this->isColumnModified(PhotoI18nPeer::CULTURE)) $criteria->add(PhotoI18nPeer::CULTURE, $this->culture);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PhotoI18nPeer::CULTURE, $this->culture);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PhotoI18n (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PhotoI18n Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PhotoI18nPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PhotoI18nPeer();
		}
		return self::$peer;
	}
	...
	{
		if ($this->aPhoto === null && ($this->id !== null)) {
			$this->aPhoto = PhotoPeer::retrieveByPk($this->id);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aPhoto->addPhotoI18ns($this);
			 */
		}
		return $this->aPhoto;
	}
	...
	{
	  if (!$callable = sfMixer::getCallable('BasePhotoI18n:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BasePhotoI18n::%s', $method));
	  }
