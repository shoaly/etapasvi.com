...
/**
 * Base class that represents a row from the 'revisionhistory' table.
 *
 * 
 *
 ...
 */
abstract class BaseRevisionhistory extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the page_mnemonic field.
	 * @var        string
	 */
	...
	const PEER = 'RevisionhistoryPeer';
	...
	/**
	 * Initializes internal state of BaseRevisionhistory object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [page_mnemonic] column value.
	 * 
	 * @return     string
	 */
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = RevisionhistoryPeer::CREATED_AT;
			}
	...
	/**
	 * Set the value of [page_mnemonic] column.
	 * 
	 * @param      string $v new value
	 * @return     Revisionhistory The current object (for fluent API support)
	 */
	public function setPageMnemonic($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RevisionhistoryPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseRevisionhistory:delete:pre') as $callable)
			{
			...
			if ($ret) {
				RevisionhistoryPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRevisionhistory:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRevisionhistory:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(RevisionhistoryPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRevisionhistory:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RevisionhistoryPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($has_non_empty_fields || ($only_service_fields_modified && !$is_i18n)) {
						$pk = RevisionhistoryPeer::doInsert($this, $con);
					...
					$affectedRows += RevisionhistoryPeer::doUpdate($this, $con);
				}
	...
	{
		$pos = RevisionhistoryPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = RevisionhistoryPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RevisionhistoryPeer::ID)) $criteria->add(RevisionhistoryPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(RevisionhistoryPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Revisionhistory (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Revisionhistory Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RevisionhistoryPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RevisionhistoryPeer();
		}
		return self::$peer;
	}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseRevisionhistory:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseRevisionhistory::%s', $method));
	  }
